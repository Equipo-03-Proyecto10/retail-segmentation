...
| **Frozen ERD including the bi-temporal segment assignment model** | Estefanía + Marcelo |
| ADR-001 Authentication and Token Lifecycle | Marcelo |
| ADR-002 Data Ownership Map | Estefanía |
| ADR-003 API Standards including the `Accept`-header XML contract | Marcelo |
| Seed dataset loaded with synthetic store, channel and injected migrations | Estefanía |
| Coding standards, naming conventions, Definition of Done published | Marcelo |

Detailed in `03-sprint-00-backlog.md`.
...
**The critical acceptance test:** run the segmentation twice with different parameters, then show that the first run's assignments are still queryable and that the migration report correctly identifies which customers moved between them. If this works, the project's central requirement is demonstrably met. If it does not, no amount of UI polish compensates.
...
| 7 | PostgreSQL model (conceptual, logical, physical) | S0 | Estefanía | `docs/data/postgresql-model.md` |
...
9. Show the migration report identifying customers who moved between runs
